-- Drop the table to ensure a clean slate, in case the previous migration partially failed.
DROP TABLE IF EXISTS public.article_jobs;

-- Re-create the table with the correct column type for site_id
CREATE TABLE public.article_jobs (
    id bigint PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
    created_at timestamptz DEFAULT now() NOT NULL,
    user_id uuid REFERENCES auth.users(id) NOT NULL,
    site_id uuid REFERENCES public.wp_sites(id) ON DELETE CASCADE NOT NULL, -- Corrected type
